...
cp "$project_dir/backend/main.py" "$project_dir/backend/__init__.py" "$project_dir/backend/memory.py" "$project_dir/backend/exe_install.py" "$package_root/backend/"
...
echo "Created lightweight $project_dir/build/DeckRecall.zip"

if test -f "$project_dir/work/release-assets/lsfg-zh.zip" && \
   test -f "$project_dir/work/release-assets/fsr4-zh.zip"; then
    ...
    echo "Created offline $project_dir/build/DeckRecall-full.zip"
else
    echo "Offline LSFG/FSR4 assets are absent; skipped DeckRecall-full.zip."
fi
